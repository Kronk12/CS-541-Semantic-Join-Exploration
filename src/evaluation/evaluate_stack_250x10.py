...
def evaluate_stackoverflow_250x10():
    df_a = pd.read_csv('data/table_a_stack_250.csv')
    df_b = pd.read_csv('data/table_b_stack_10.csv')
    ...
    for ratio in target_ratios:
        ...
        for force_proj in projection_states:
            print("\n" + "="*60)
            print(f"Running 250x10 StackOverflow | ratio: {ratio} (Divisor: {divisor}) | Projection: {force_proj}")
            print("="*60)
            ...
            output_filename = f"src/results/sim_logs/stackoverflow_250x10_master_log_ratio_{ratio}{suffix}.json"
