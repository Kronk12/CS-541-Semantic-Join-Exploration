...
def semantic_join(
    ...
    if force_strategy is not None:
        jstrat = force_strategy
        plan_notes.append(f"join_strategy={jstrat} (Forced)")
    elif embedding is None and clustering is None:
    ...
    if jstrat == "classifier":
        if force_labels is not None:
            labels = force_labels
            plan_notes.append("labels=Forced via parameters")
        else:
            labels = advisor.generate_classification_labels(
                predicate, table_b, schema_b, llm_model
            )
            plan_notes.append(f"labels=Generated by Advisor ({len(labels)} categories)")
            
        if not labels:
            raise ValueError("Classifier strategy selected, but no labels were generated.")
    ...
    if do_projection:
        ...
        samples_b = table_b.head(25).to_dict(orient="records")
    ...
    if method == "kmeans":
        if cluster_ratio is not None:
            ...
            k_a = min(len(table_a), max(2, len(table_a) // cluster_ratio))
            k_b = min(len(table_b), max(2, len(table_b) // cluster_ratio))
